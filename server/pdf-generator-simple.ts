 ...
 */
function addLogoToHeader(doc: PDFDocument): void {
  try {
...
/**
 * Versão simplificada da função de geração de PDF para orçamentos
 */
export async function generateQuotePDF(data: QuoteData): Promise<string> {
  console.log("Iniciando geração de PDF (versão melhorada)");
  ...
  return new Promise<string>((resolve, reject) => {
    try {
      ...
      // Título do documento com uma linha abaixo
      doc
        .fontSize(18)
        .fillColor('#1a56db')
      ...
      // Área para observações e termos
      doc
        .fontSize(10)
        .fillColor('#666666')
        ...
        .moveDown(0.5)
        .fontSize(9)
        .text('• Os preços incluem mão de obra e material, conforme especificado nos itens acima.')
        .text('• Valores sujeitos a alteração após visita técnica.')
        .text('• Condições de pagamento: a combinar.')
      ...
      doc
        .fontSize(10)
        .text('Responsável', 150, signatureY + 45, { align: 'center' })
        .text('Cliente', 400, signatureY + 45, { align: 'center' });
...
/**
 * Versão simplificada da função de geração de PDF para ordens de serviço
 */
  ...
  return new Promise<string>((resolve, reject) => {
    try {
      ...
      doc
        .fontSize(24)
        .fillColor('#1a56db')
        .text('SAM CLIMATIZA', { align: 'center' })
        .fontSize(12)
        .fillColor('#666666')
        .text('Sistema de Gestão para Climatização', { align: 'center' })
        .moveDown(1);
      ...
      if (items && items.length > 0) {
        ...
        const tableWidths = [40, 320, 50, 90];
        const colPositions = [
          50, // Item
        ...
        doc
          ...
          .text('Tipo', colPositions[3], tableTop + 6, { width: tableWidths[3], align: 'left' });
        ...
        items.forEach((item, i) => {
          ...
          doc
            ...
            .text(item.quantity.toString(), colPositions[2], rowY + 8, { width: tableWidths[2], align: 'center' })
            .text(typeText, colPositions[3], rowY + 8);
      ...
      // Área para assinaturas com linhas
      doc
        .fontSize(12)
        .fillColor('#1a56db')
      ...
      doc
        .fontSize(10)
        .fillColor('#666')
        .text('Assinatura do Técnico', 140, signatureY + 45, { align: 'center' })
        .text('Assinatura do Cliente', 390, signatureY + 45, { align: 'center' });
